...
// Remove old recurring engine at end of file
const engineStart = content.lastIndexOf('<script id="ims-recurring-engine">');
const engineEnd = content.indexOf('</script>', engineStart) + 9;
if (engineStart > -1) {
  content = content.slice(0, engineStart) + content.slice(engineEnd);
  console.log('Removed old engine');
}

// Inject before </head> — guaranteed to execute before DOMContentLoaded
...
window.imsRecurringShowPanel=function(){var ex=document.getElementById('ims-recurring-panel');if(ex){ex.remove();return;}var p=document.createElement('div');p.id='ims-recurring-panel';p.style.cssText='position:fixed;top:60px;right:16px;width:360px;background:var(--card);border:1px solid var(--border);border-radius:10px;z-index:9999;box-shadow:0 8px 32px rgba(0,0,0,.4);overflow:hidden;';p.innerHTML='<div style="background:#0B3D91;padding:12px 16px;display:flex;justify-content:space-between;align-items:center;"><div style="font-size:13px;font-weight:700;color:#fff;">Recurring Audit Engine</div><button onclick="document.getElementById(\'ims-recurring-panel\').remove()" style="background:transparent;border:none;color:#fff;font-size:18px;cursor:pointer;">&times;</button></div><div style="padding:14px;">'+(window.RECURRING_TEMPLATES||[]).map(function(t){return'<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 10px;background:var(--raised);border-radius:6px;margin-bottom:6px;"><div><div style="font-size:11px;font-weight:600;color:var(--t1);">'+t.dept+'</div><div style="font-size:9px;color:var(--t3);">'+t.type+'</div></div><span style="font-size:9px;padding:2px 8px;background:rgba(59,130,246,.12);color:#3B82F6;border-radius:10px;">'+t.freq+'</span></div>';}).join('')+'<div style="margin-top:10px;display:flex;gap:8px;"><button onclick="imsRecurringGenerateAll();document.getElementById(\'ims-recurring-panel\').remove()" style="flex:1;background:#0B3D91;border:none;color:#fff;font-size:11px;font-weight:700;padding:8px;border-radius:6px;cursor:pointer;">Generate Next Cycle</button><button onclick="document.getElementById(\'ims-recurring-panel\').remove()" style="background:var(--raised);border:1px solid var(--border);color:var(--t2);font-size:11px;padding:8px 12px;border-radius:6px;cursor:pointer;">Cancel</button></div></div>';document.body.appendChild(p);};
...
if (headClose > -1) {
  ...
  console.log('Injected at </head> position:', headClose);
}
