...
// Correct sb-more-items block (extracted from operations)
const CORRECT_BLOCK = `<div id="sb-more-items" style="display:none">
...
PAGES.forEach(p => {
  ...
  // Find end of sb-more-btn closing </div>
  const btnEnd = html.indexOf('</div>', html.indexOf('<span>More</span>')) + 6;

  // Find end of last sb-item (all loose items after btn)
  let lastItemEnd = btnEnd;
  let idx = btnEnd;
  while ((idx = html.indexOf('<a class="sb-item"', idx + 1)) !== -1 && idx < btnEnd + 4000) {
    lastItemEnd = html.indexOf('</a>', idx) + 4;
  }

  if (lastItemEnd === btnEnd) {
    console.log('SKIP — no sb-items found after btn in:', p);
    return;
  }

  // Replace bare items with correct wrapped block
  html = html.substring(0, btnEnd) + '\n    ' + CORRECT_BLOCK + html.substring(lastItemEnd);

  fs.writeFileSync(p, html, 'utf8');

  // Verify
  const links = [...html.matchAll(/href="(safetypro_[^"]+)"/g)].map(m => m[1]);
  const moreLinks = links.filter(l => ['audit_compliance','field','hrm','ai','auditor','documents','admin'].some(k => l.includes(k)));
  console.log('PATCHED:', p);
  console.log('  sb-more-items:', html.includes('sb-more-items'));
  console.log('  links:', [...new Set(moreLinks)].join(', '));
