...
(function(){
  ...
  var METHODOLOGY_DB = {
    ...
    "critical-risks": {
      ...
      competitiveAdvantage: "SafetyPro AI's Risk Register follows ISO 31000 + ISO 31010 with full HIRA + Environmental Aspect + Method Statement workflow. Unlike SafetyCulture's checkbox approach, SafetyPro provides quantified 5x5 risk scoring with integrated mitigation tracking. Default matrix customizable via Admin -> Risk Framework (3x3, 4x4, or NxN)."
    },
    ...
    "overdue-actions": {
      ...
      competitiveAdvantage: "Real-time overdue tracking across ALL action types (CAPAs, audit findings, observations, regulatory responses) in a unified counter. Competitors like SafetyCulture track only observation actions separately; Intelex requires custom queries. SafetyPro AI auto-aggregates across all modules with role-based escalation - auto-notifies supervisor/manager/executive based on overdue age."
    },
    ...
    "open-incidents": {
      ...
      competitiveAdvantage: "Unified incident register spanning injury + property + environmental + process safety events, mapped to multiple global reporting frameworks simultaneously. Auto-generates OSHA 300/300A logs, ILO notifications, RIDDOR reports, and API RP 754 process safety indicators from the same source data. Competitors require separate workflows for each jurisdiction."
    },
  ...
  ,
    ...
    "top-performers": {
      ...
      competitiveAdvantage: "Holistic 5-dimension scoring rewards proactive behavior, learning, and culture-building - not just closure rate. Customizable per tenant and culture (Peer Recognition component adjustable for collectivist/hierarchical cultures). Incident record caps at 0 (no negative scoring) to encourage reporting. Competitors like Intelex rank by closure rate alone - misses the holistic HSE engagement picture enterprise buyers want to measure."
    }
  ,
    ...
    "closed-this-week": {
      ...
      competitiveAdvantage: "Real-time weekly closure tracking complements Overdue Actions (trailing) with forward momentum visibility. Unlike competitors that only show monthly/quarterly closure counts, SafetyPro AI provides week-level cadence matching modern agile HSE management rhythms."
    },
    ...
    "escalated": {
      ...
      competitiveAdvantage: "Automated 4-level escalation chain (Supervisor -> Manager -> Senior Mgmt -> Executive) with both age-based and severity-based triggers. Competitors require manual escalation; SafetyPro AI's rule engine ensures nothing critical falls through the cracks. Role-based notifications auto-route to the right management level without human intervention."
    }
  };
